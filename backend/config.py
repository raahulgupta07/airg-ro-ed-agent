...
APP_CHANGELOG = [
    "2026.9.6 — The item-count guard was reading a quantity the reviewer never sees. At the point it runs, a row's quantity is still the raw string off the form (`94 CT`, `1X144`), and treating any non-empty string as a quantity made every echo row look genuine — so on production the guard declined on exactly the documents it was written for, while every test passed. Those same rows reach the database with a NULL quantity, because the shared parser refuses a unit-suffixed number. The guard now reads the quantity through that same parser, so it judges the row that will actually be stored.",
    "2026.9.5 — The item-count guard was counting the wrong list. It ran before the reconcile gate, and the surplus rows are ADDED by the recovery pass inside that gate — one document went from 8 rows to 13 there — so on three of the four documents it was written for it saw nothing to do and said so. Moved after recovery, with the verdict recomputed on the rows that actually ship: 100325137140 now stores 5 rows for a form that prints 5, and 100329052130 stores 3 and reconciles to the penny, the 0.18%% gap having been the two phantom rows all along. 100327095522 returns 13 rows against a printed 6 and every row carries a quantity, so it is flagged for a human rather than guessed at — choosing which real-looking row is the intruder is exactly what this guard refuses to do.",
    "2026.9.4 — Four fixes from the 2 Sep complaint round. (1) The form prints its own item count — `Total items 3` — and it was never read. Three documents came back with more rows than the declaration has (9 for 5, 8 for 6, 5 for 3); the surplus rows were echoes carrying no quantity, and on one document their values summed to 197,001, which was EXACTLY the item-sum gap. A row is now dropped only when the form printed a count, the row has no quantity, AND its figure appears nowhere in the document text — never below the printed count, and never on a scanned bundle where nothing can be corroborated. (2) A total that spells out the invoice-currency amount IS that amount: bundle 0259100560 stored 942,418,932 for a printed `CIF- 942,418.9320`, the same digits with the decimal point lost, on twelve photographed pages where no reader could settle it. The digits settle it, and the item block supplies the real total. (3) The checks panel warned \"Freight cost is empty\" and \"Insurance cost is empty\" on every document, because every one of these forms prints a dash there — two permanent entries teach a reviewer to skip the list, so they now appear only when the CIF identity actually fails. (4) The backup sidecar had restarted 240 times: `expr N + 0` exits 1 when the result is zero, `set -e` turned that into an exit, so at any minute :00 the loop died and the container restarted straight into another full dump. And the boot banner no longer prints the database password.",
    "2026.9.3 — The scroll belongs to the PDF pane, not the window. Stacking every page made the viewer panel as tall as the whole document, so scrolling the pages scrolled the BROWSER and carried the declaration table away off the top — side-by-side review with only one side on screen. The pane now holds a fixed height and scrolls internally, and it sticks in place while the data column scrolls on its own. Two CSS facts did the damage: a flex child will not shrink below its content without min-height 0, and grid items stretch to the tallest row unless told to align to the start, which left the sticky pane nothing to slide against.",
