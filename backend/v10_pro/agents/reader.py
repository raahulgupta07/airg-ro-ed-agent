...
from v10_pro.config import READER_MODEL, OPENROUTER_API_KEY


READER_SYSTEM = """You are a Myanmar Customs document reader. Extract every field
from CUSDEC-1 / MACCS Release Order PDFs.

HANDWRITING-SPECIFIC INSTRUCTIONS (V10 PRO):

CHAIN-OF-THOUGHT (mandatory before output):
1. First describe the document layout (which boxes you see).
2. State Box 11 (Declaration No) location.
3. List each digit of declaration number ONE BY ONE: "I see digit 1: '1', digit 2: '0', ..."
4. State expected length: 11-12 digits for Myanmar customs declarations.
5. Then output the JSON.

CONFUSABLE-PAIR PRIMER:
Common handwriting confusions: 0<->8, 1<->7, 3<->8, 5<->6, 9<->0.
If unsure between 2 candidates, output "UNCERTAIN_<a>_OR_<b>" not a guess.
Never silently pick — flag the ambiguity.

BURMESE-SCRIPT NOTE:
Field labels may be in Burmese (မြန်မာ).
NUMBERS are ALWAYS Arabic digits 0-9 (not Burmese ၁၂၃၄).
SKIP reading any Burmese text — focus only on Arabic digits + Latin words.

LENGTH SANITY:
- Declaration No: ALWAYS 11-12 digits. If you read fewer, you missed digits — RE-EXAMINE.
- Exchange rate: must match currency band (THB 40-110, USD 1300-5500).
- HS code: 8-10 digits format NNNN.NN.NN NN.
...
def _quick_importer_extract(pdf_path: str) -> Optional[str]:
    """Cheap haiku call to extract just the importer name from page 1."""
    import requests
    from v10_pro.config import HAIKU, OPENROUTER_API_KEY as _KEY
    ...
    payload = {
        ...
        "messages": [{"role": "user", "content": [
            ...
            {"type": "text", "text":
                "Extract ONLY the importer/consignee company name from this Myanmar customs PDF.\n"
                "Look in Box 8 ('Importer'). Return strict JSON only:\n"
                "{\"importer\": \"COMPANY NAME\"}\n"
                "If unclear, return {\"importer\": null}."},
        ]}],
        "temperature": 0,
        "max_tokens": 200,
    }
    try:
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                record_cost(label="reader_importer", model=HAIKU, usage=_j.get("usage", {}))
            except Exception:
                pass
            raw = _j["choices"][0]["message"]["content"]
            parsed = _parse_json(raw)
            if parsed and parsed.get("importer"):
                return str(parsed["importer"]).strip()
    except Exception as e:
        print(f"[Reader-importer] error: {e}")
    return None
...
def read_pdf(pdf_path: str) -> Optional[Dict]:
    ...
    # Quick importer extract + memory load (2-pass approach)
    importer = _quick_importer_extract(pdf_path)
    memory_block = ""
    if importer:
        try:
            from v10_pro.tools.memory_tools import get_importer_profile, build_memory_prompt_block
            profile = get_importer_profile(importer)
            memory_block = build_memory_prompt_block(profile)
            print(f"[Reader] memory loaded for '{importer}': "
                  f"{profile.get('n_past_extractions', 0)} past")
        except Exception as e:
            print(f"[Reader] memory load failed: {e}")

    user_text = (memory_block + "\n\n" + READER_PROMPT) if memory_block else READER_PROMPT
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                try:
                    from v10_pro._cost_tracker import record_cost
                    record_cost(label="reader_main", model=READER_MODEL, usage=j.get("usage", {}))
                except Exception:
                    pass
