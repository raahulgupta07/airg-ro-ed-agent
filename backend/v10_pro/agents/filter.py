...
from v10_pro.config import FILTER_MODEL, OPENROUTER_API_KEY
...
def filter_pages(pdf_path: str) -> Tuple[List[int], Dict]:
    ...
    try:
        ...
        try:
            ...
            usage = {}
            for attr in ("usage", "metrics", "response_usage"):
                v = getattr(resp, attr, None)
                if v:
                    if isinstance(v, dict):
                        usage = v
                    elif hasattr(v, "__dict__"):
                        usage = {k: getattr(v, k, None) for k in ("cost", "total_cost",
                                                                    "prompt_tokens",
                                                                    "completion_tokens")}
                    break
            record_cost(label="filter", model=FILTER_MODEL, usage=usage)
        except Exception:
            pass
